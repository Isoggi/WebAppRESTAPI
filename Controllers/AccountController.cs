...
using WebAppRESTAPI.Models.Static;
...
namespace WebAppRESTAPI.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] CreateLoginViewModel model)
        {
            var userExists = await _userManager.FindByNameAsync(model.Username);
            if (userExists != null)
                return StatusCode(StatusCodes.Status500InternalServerError, new  { Status = "Error", Message = "User already exists!" });
        ...
        [HttpPut("{id}")]
        [Authorize(Roles = UserRoles.Admin)]
        public void UpdateRole(string id,[FromBody] Account account)
        {
